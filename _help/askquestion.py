...
class AskQuestion:
    def __init__(self, r, rself, *args, **kwargs):
        ...
        im_p            = os.path.join(gv.application_path, "icons", "cus", "question-mark-48.png")
        ...
        but1             = get_a_button(master, text=self.rself.btext1, row=2, column=1, use_ttk=False, padx=0, pady=10, width=10, ipady=0, font=('Calibri', 10, 'bold'), ipadx=0, bg=self.rself.bback1 if self.rself.bback1 else "#37A000")
        but1.config(command=lambda: self.destroy(1))
        but2             = get_a_button(master, text=self.rself.btext2, row=2, column=2, use_ttk=False, padx=0, pady=10, width=10, ipady=0, font=('Calibri', 10, 'bold'), ipadx=0, bg=self.rself.bback2 if self.rself.bback2 else "#FF0000")
        ...
        if self.rself.btext1: but1.config(text=self.rself.btext1)
        if self.rself.btext2: but2.config(text=self.rself.btext2)
        if self.rself.title: master.title(self.rself.title)
