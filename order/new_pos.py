...
# gv.error_log(str(f"File: order/new_pos.py"))

class PosOrder:
    def __init__(self, realself, *args, **kwargs):
        super(PosOrder, self).__init__(*args, **kwargs)
        ...
        gv.pos_order = self
        gv.order_sys = self
        self.update = realself.update = False

        gv.order_sys_realself = self.realself
        gv.clear_order = clear
    ...
    def get_dependency_master(this):
        ...
        self.upcell = PanedWindow(self.new_order_paned, height=gv.h(550), orient='horizontal')
        self.downcell = PanedWindow(self.new_order_paned, height=96, row=1, orient='horizontal')

        self.new_order_paned.add(self.upcell)
        self.new_order_paned.add(self.downcell)
