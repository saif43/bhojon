...
from database.table import (
			...
			PaymentMethod,
			CustomerOrder,
			Bill, BillCardPayment,
			MultiPay, CashRegisterPaymentHistory
		)
...
# gv.error_log(str(f"File: order/snippets/payment.py"))

class PosOrderPayment:
	def __init__(self, fself, realself, order=None, *args, **kwargs):
		super(PosOrderPayment, self).__init__(*args, **kwargs)

		# gv.error_log(str(f"self: {self} --> realself: {realself} --> fself: {fself} --> order: {order} --> args: {args} --> kwargs: {kwargs}"))

		self.fromself = fself
		lf = len(order)
		self.foods_canv_h = (lf+1)*28 if lf < 4 else 112
		...
		realself.order_payment_toplevel = self.master = master = Toplevel(
										width=self.res_width, \
										title="Pay money for selected order", \
										height=self.res_height+self.foods_canv_h, \
										resize_x=1, \
										resize_y=1
									)

		self.realself = realself
		self.allorder = self.order = order
		self.select_list = {i: v for i, v in enumerate(order)}
		...
		# master.bind_all(
		# 				'<KeyRelease>', lambda e, \
		# 				itr=self.current_amnt_box, \
		# 				att='box': self.count_after_type(
		# 					e, itr, att
		# 				)
		# 			)
	...
	def get_dependency_master(self):
		self.modal_pained = PanedWindow(self.master, width=self.res_width-40)

		self.modal_head = PanedWindow(self.modal_pained, height=self.foods_canv_h, width=self.res_width-10)
		self.modal_body = PanedWindow(self.modal_pained, orient='horizontal')

		self.modal_pained.add(self.modal_head)
		self.modal_pained.add(self.modal_body)

		self.modal_head_canv = Canvas(
								self.modal_head, width=620, gridrow=0,
								gridcolumn=0, highlightthickness=0, pady=12
							)

		self.modal_head.add(self.modal_head_canv)
	...
	def get_orders_detail(self):
		xlist = [[0, 160], [160, 360], [360, 560], [560, 760], [760, 948]]

		hlist = [
			'Select', 'Order Number', 'Total Amount (%s)' %gv.st['curr_icon'],
			'Paid Amount (%s)' %gv.st['curr_icon'],
			'Due (%s)' %gv.st['curr_icon']
		]

		checked_mark = os.path.join(
								gv.fi_path, "cus", "check-mark-3-20.png"
							)

		unchecked_mark = os.path.join(
								gv.fi_path, "cus", "check-mark-3-24 (1).png"
							)
		...
		for ix, i in enumerate(hlist):
			x = xlist[ix]

			self.modal_head_canv.create_rectangle(
								x[0], 0, x[1], 28,
								fill='#FFFFFF',
								outline='#DDDDDD'
							)

			self.modal_head_canv.create_text(
								x[0]+12,  14, text=i,
								font=('Calibri', 11, 'bold'),
								anchor='w'
							)

		for en, ordr in enumerate(self.order):
			en28 = (en+1)*28

			globals()["pcm_%s" %en] = PhotoImage(
								file=checked_mark
							).subsample(2, 3)

			globals()["pucm_%s" %en] = PhotoImage(
								file=unchecked_mark
							).subsample(10, 10)

			tot = ordr['totalamount']
			cpaid = ordr['customerpaid']

			due = float(tot)-float(cpaid)
			due = due if due>0 else 0

			ic = "#FFFFFF"
			im = globals()["pucm_%s" %en]

			if en in self.select_list:
				ic = "#0078D7"
				im = globals()["pcm_%s" %en]

				self.amounts['total'] += tot
				self.amounts['paid'] += cpaid
				self.amounts['due'] += due

			hlist = [
				globals()["pcm_%s" %en], ordr['id'],
				tot, cpaid, float(tot)-float(cpaid)
			]
			...
			for ix, i in enumerate(hlist):
				...
				self.modal_head_canv.create_rectangle(
										x[0], en28, x[1], en28+28,
										fill='#FFFFFF',
										outline='#DDDDDD'
									)

				if type(i) == PhotoImage:
					globals()["%sbtrec_%s" %('pcm_', en)] = self.modal_head_canv.create_rectangle(
										32, en28+5, 56, en28+23,
										fill=ic, outline="#37A000"
									)

					globals()["%sbtim_%s" %('pcm_', en)] = self.modal_head_canv.create_image(
										44, en28+14, image=im, anchor="center"
									)
				else:
					self.modal_head_canv.create_text(
										x[0]+12,  en28+14, text=i,
										font=('Calibri', 11), anchor='w'
									)

				for i in ['pcm_btrec_', 'pcm_btim_']:
					self.modal_head_canv.tag_bind(
										globals()["%s%s" %(i, en)], "<Button-1>",
										lambda e, itm=en, o=ordr: \
										self.toggle_select(itm, o)
									)

			self.modal_head_canv.config(
								height=en28+28,
								scrollregion=[0,0,948,en28+28]
							)

		change = self.amounts['paid'] - self.amounts['total']
		self.amounts['change'] = change if change>0 else 0

	def pay_order_amount(self):
		lofmeths = len(self.methods)
		totsordr = len(self.select_list)
		...
		if lofmeths == 0:
			_help.messagew(
					root=gv.rest.master,
					msg1=ltext("payment not complete"),
		...
		if sosp or somp:
			...
			up_ord_list, up_bi_list = [], []
			ppaid = 0
			tpaid = self.amounts['paid']

			for ordr in self.select_list.values():
				canpay = tpaid - ppaid
				ordram = ordr['totalamount']

				npaid = ordram if canpay>=ordram else (canpay if canpay>0 else 0)
				ppaid += npaid

				bill = Bill().qset.filter(order_id=ordr['id']).first()

				sord = {
					'marge_order_id': self.merge_id,
					'customerpaid': npaid,
					'order_status': 4,
					'saleinvoice': ordr['saleinvoice'],
					'sync_status': 0
				}

				up_ord_list.append(sord)
				...
				up_bi_list.append(sbi)

			CustomerOrder().qset.update_all(up_ord_list, where='saleinvoice')
			Bill().qset.update(up_bi_list, where='id')

		orders = CustomerOrder().qset.filter(order_status=1).all()

		ords = ["%s/%s\t%s %s\t%s%s" %(
						o['saleinvoice'], o['order_id_online'],
						o['order_date'], o['order_time'], gv.st['curr_icon'],
						o['totalamount']) for o in orders
				]

		if getattr(self.realself, 'onord_search', 0):
			self.realself.onord_search.show_selection(values=ords)

			gv.paginator(
					self.realself,
					self.realself.ong_ord_footer,
					orders,
					self.realself.ong_ord_li_canvas,
					gv.goo_li_cont
				)
		...
		allsinv = ','.join(o['saleinvoice'] for o in self.order)

		GeneratePosInvoice(
				realself=self.realself,
				invoice_id=allsinv.split(',')
			)

		inv_file = os.path.join(
				gv.invoice_path,
				"%s_pos_invoice.pdf" %allsinv
			)

		PrintInvoice(realself=self.realself, print_file=inv_file)

	def pay_to_web(self):
		order = {
			ix:r for ix,r in self.select_list.items() \
			if r['sync_status']}.get(0, self.select_list[0]
		)
	...
	def toggle_select(self, itm, order):
		if itm in self.select_list:
			self.select_list.pop(itm)
		else:
			self.select_list[itm] = order
	...
	def method_update(self, method):
		print(self.methods[method])
