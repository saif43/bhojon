...
from database.table import CustomerInfo, Employee, QROrder, Tables
from ntk import PanedWindow, Frame, Canvas, Scrollbar, Label, Entry

# gv.error_log(str(f"File: order/list_qr.py"))

class QROrderList:
    def __init__(self, realself, *args, **kwargs):
        super(QROrderList, self).__init__(*args, **kwargs)
    ...
    def get_dependency_master(this):
        ...
        self.qr_or_head_holder = Frame(self.new_order_paned, width=gv.device_width - 80, height=44, pady=0)
        self.qr_or_head = Frame(self.qr_or_head_holder, width=gv.device_width - 80, height=44, pady=0, sticky="e")
        
        self.qr_or_content = PanedWindow(
            self.new_order_paned, pady=5, row=1, width=gv.device_width - 42, height=gv.device_height - 120)
        
        self.new_order_paned.add(self.qr_or_head_holder)
        self.new_order_paned.add(self.qr_or_content)
        
        self.qr_or_header_frame = Canvas(self.qr_or_content, width=gv.device_width - 42, height=44, mousescroll=False)
        self.qr_or_canv_holder = Frame(
            self.qr_or_content, width=gv.device_width - 42, height=gv.device_height - 288, row=1, pady=10)
        
        self.qr_or_footer_frame_holder = Frame(self.qr_or_content, row=2, width=gv.device_width-80)
        self.qr_or_footer_frame = Frame(self.qr_or_footer_frame_holder, pady=0, padx=0, sticky='e')
        
        self.qr_or_content.add(self.qr_or_header_frame)
        self.qr_or_content.add(self.qr_or_canv_holder)
        self.qr_or_content.add(self.qr_or_footer_frame_holder)
        
        self.qr_or_canvas = Canvas(
            self.qr_or_canv_holder, width=gv.device_width - 42, height=gv.device_height - 288,
            scrollregion=[0, 0, 1160, 0], highlightbackground="#FAFAFA")
        self.scroll_order_list = Scrollbar(self.qr_or_canv_holder, self.qr_or_canvas, column=1)
        
        # self.qr_or_canv_holder.add(self.qr_or_canvas)
        # self.qr_or_canv_holder.add(self.scroll_order_list)

        this.get_order_list_head(self, self.qr_or_head)
    ...
    def order_list_popup_callback(this, self, event=None, view=False, view_invoice=False, data=None):
        if view:
            ...
            self.view_order_detail_window = ViewInvoice(self, self.viewordertoplevel, data['invoice'], qr=True)
            ...
            self.view_order_invoice_window = ViewPosInvoice(
                self, self.vieworderinvoicelevel, data['invoice'], pos_invoice=True, qr=True)
    ...
    def get_order_list_head(this, self, master):
        ...
        self.olhs_e.bind(
            "<KeyRelease>", lambda e, funct=self.qr_or_footer_frame,
                                   canv=self.qr_or_canvas, target=this.get_order_list_content,
        ...
        gv.paginator(self, self.qr_or_footer_frame, orders, self.qr_or_canvas, this.get_order_list_content)
        
        gv.oo_mas = self
        gv.oo_foot = self.qr_or_footer_frame
        gv.oo_canv = self.qr_or_canvas
    ...
    def get_order_list_content(this, self, master, orders=None, all=None):
        ...
        this.olh_frame = self.qr_or_header_frame
        ...
        self.total_qr_order_count.config(text="{}".format(len(all)))
        ...
        if len(orders) == 0:
            ...
            gv.w(1324)/2, 17, text="{}".format(ltext("no_order_found")), width=200,
            font=("Calibri", 10, "bold"), fill="#374767")
